...
def sub_bytes(block):
    ...
    block_bin = str(bin(block))
    print('BLOCK_BIN: ' + block_bin)

    # remove the '0b' from the binary block string
    block_bin = block_bin[2:]

    # add leading zeroes to the binary block string to prep for split
    needed_zeroes = 8 - len(block_bin)
    for x in range(0, needed_zeroes):
        block_bin = '0' + block_bin

    # print the binary block string
    print()
    for x in range(0, len(block_bin)):
        print(block_bin[x], end='')


def main():
    sub_test_hex = 0x19
    sub_bytes(sub_test_hex)
